st_seed <- as.integer(ymd('2020-11-24'))
set.seed(st_seed)
readRDS(here::here('prj_dbdf', dta.names$f_cpt_list[3]))

# -- Run method 8 - Ap cluster
library(apcluster)

cat('Begin AP cluster randomization procedure....', '\n')
st_time <- Sys.time()
  ...
  saveRDS(res_iter, here::here('prj_dbdf', dta.names$f_rand_res[8])) 
